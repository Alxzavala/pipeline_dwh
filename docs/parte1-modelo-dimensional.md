...
- **Column-level security implementada** (no solo recomendada): un policy tag de Data Catalog (`dwh-pii-taxonomy / PII - Datos de contacto`) aplicado sobre `correo`, `telefono` y `direccion` en `dim_cliente` (`terraform/data_governance.tf`). Solo el Service Account del pipeline tiene el rol `datacatalog.categoryFineGrainedReader` sobre ese policy tag (necesario para la detección de cambios SCD2); cualquier otro principal —incluida la cuenta administradora del proyecto— recibe `Access Denied` al consultar esas columnas, verificado en vivo. Nota: BigQuery también soporta una *Data Masking Policy* que sustituye el valor por un hash en vez de negar el acceso, pero ese recurso (`google_bigquery_datapolicy_data_policy`) requiere que el proyecto pertenezca a una organización de Cloud Identity/Workspace; esta prueba corre sobre una cuenta personal sin organización, así que el control queda en modo "denegar acceso" en vez de "enmascarar valor".
